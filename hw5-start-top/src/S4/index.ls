    ...
    apb[0].onclick =!->
        for item in buttons
            if item.status === 0
                return
        reset-all!
        robot-auto-click 0,generate-sequence!

#-----------------------    S4   ----------------------------------#

generate-sequence =->        # 生成随机序列
    sequence = ['A' to 'E'].sort -> 0.5-Math.random!
    show-sequence sequence
    return sequence

show-sequence =(sequence)->   # 显示序列
    div = $ "<div class='sequence'>"+sequence+"</div>"
    $ '#info-bar' .append div

robot-auto-click =(i,sequence)->   #模拟点击
    buttons = $ '.button'
    if i >= buttons.length
        return
    info = $ '.info'
    index = sequence[i].char-code-at! - 'A'.char-code-at!
